...
impl RpcClient {
    pub async fn new(rpc_url: &str, chain_id: u64) -> Result<Self> {
        ...
        if chain.as_u64() != chain_id {
            return Err(ArgusError::RpcError(
                format!("Chain ID mismatch: expected {}, got {}", chain_id, chain.as_u64())
            ));
        }
    ...
    pub async fn get_typical_swap_gas(&self) -> Result<u64> {
        ...
        if self.chain_id == 1 {
            ...
            Err(ArgusError::RpcError(format!("Unsupported chain ID: {}", self.chain_id)))
        }
    }
    ...
    pub async fn get_max_priority_fee_per_gas(&self) -> Result<u64> {
        ...
        let priority_fee = self.provider.request::<_, U256>(
    ...
    pub async fn estimate_l1_data_fee(&self, _to_address: Address, calldata: Vec<u8>) -> Result<u64> {

        if self.chain_id != 8453 {
            return Ok(0);
        }
        
        let oracle_address: Address = "0x420000000000000000000000000000000000000F".parse().unwrap();
